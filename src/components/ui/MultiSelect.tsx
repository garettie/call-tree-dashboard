...
export const MultiSelect: React.FC<MultiSelectProps> = ({ label, options, selected, onChange, placeholder = "Select..." }) => {
  ...
  return (
    <div className="relative w-full" ref={containerRef}>
      ...
      <div 
        className={cn(
          "min-h-[38px] w-full bg-white/80 backdrop-blur-sm border rounded-lg px-2 py-1 cursor-pointer flex items-center justify-between transition-all duration-200 shadow-sm hover:shadow-md",
          isOpen ? "border-accent-primary ring-2 ring-accent-primary/20" : "border-gray-200 hover:border-accent-primary/50"
        )}
          ...
          <div className="overflow-y-auto max-h-60">
            {filteredOptions.length === 0 ? (
                ...
                <div 
                    ...
                    selected.includes(option) && "bg-accent-light/50 text-accent-primary font-medium"
                    )}
                    onClick={() => toggleOption(option)}
                >
                    <div className={cn(
                    "w-4 h-4 border rounded mr-2 flex items-center justify-center transition-colors",
                    selected.includes(option) && "bg-accent-primary border-accent-primary",
